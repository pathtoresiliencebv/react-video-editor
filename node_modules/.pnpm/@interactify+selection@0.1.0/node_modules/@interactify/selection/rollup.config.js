...
try {
  const licenseFile = fs.readFileSync(process.cwd() + '/LICENSE', {
    encoding: 'utf8',
  });
...
function config(options) {
  if (Array.isArray(options)) {
    return options
      .map((options2) => config(options2))
      .reduce((prev, cur) => prev.concat(cur), []);
  }
  if (Array.isArray(options.output)) {
    return options.output.map((file) =>
      config({
        ...options,
        output: file,
      }),
  ...
  if (uglify) {
    const condition =
      typeof uglify === 'string'
        ? uglify
        : `name:(\\s*)${pkg.name.replace(/\//g, '\\/')}`;
